...
from smoothing_net import SmoothingNet
from dataset import DataLoaderForSN
...
def train(config):
	...
	for epoch in range(config.num_epochs):
		for iteration, data in enumerate(train_loader):
			...
			denoise, out = net(rgb, nir, nir_mask)

			loss_edge = edge_loss(out, gt, nir_mask)
	...
	torch.save(net.state_dict(), config.snapshots_folder + 'smoothing_cnn.pth') 
